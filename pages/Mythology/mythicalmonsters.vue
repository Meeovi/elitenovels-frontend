<template>
    <div class="categoryPage">
      ...
      <v-row style="background-color: lightgreen;">
        <v-col cols="12">
          <h1 class="characterHeader">Mythical Monsters</h1>
        </v-col>
      </v-row>
  
      <v-row>
        <v-col cols="3" v-for="mythical in data.CharacterItems.items" :key="mythical">
        <v-card class="mx-auto" max-width="300">
          <img class="align-end text-white" height="350" :src="`${mythical.content.image.filename}`" cover />

          <v-card-title>{{ mythical.content.name }}</v-card-title>

          <v-card-actions>
            <v-btn color="blue" variant="outlined" :href="`/mythology/${mythical.id}`">
              Explore
            </v-btn>
          </v-card-actions>
        </v-card>
  ...
  const query = gql `
  query {
  CharacterItems (with_tag:"MythicalMonsters"){
    items {
      id
      content {
        name
