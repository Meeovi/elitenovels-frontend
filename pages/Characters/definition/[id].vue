...
<script setup>
    ...
    // Get the specific dictionary and its definition
    const {
        data: dictionary
    } = await useAsyncData('dictionary', () => {
        return $directus.request($readItem('dictionary', route.params.id, {
            fields: ['*', '*.*.*'],
        }))
    })

    useHead({
        title: computed(() => dictionary.value?.name || 'Definition Page')
    })
</script>